...
from model.RecallMamba import RecallMamba
...
# Placeholder for total training time start
...
def train(
    ...
    print_log(f"Total Training Time: {total_training_time:.2f} s", log=log)
    print_log(f"Average Time per Epoch: {avg_epoch_time:.2f} s", log=log)
...
if __name__ == "__main__":
    ...
    model_name = RecallMamba.__name__

    with open(f"{model_name}.yaml", "r") as f:
        cfg = yaml.safe_load(f)
    cfg = cfg[dataset]

    model = RecallMamba(**cfg["model_args"])
